...
import com.yimeinew.presenter.CommFast2Presenter;
...
public class Fast2Activity extends BaseActivity implements CommFastView {
    ...
    private CommFast2Presenter commPresenter;

    public static final String Title = "快速";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        zcno=zCnoInfo.getId();
        this.setTitle(Title+"->二次"+zCnoInfo.getName());
        zcAdapter = new ArrayAdapter<Pair>(this, android.R.layout.simple_spinner_item, zcAdapterData);
        zcSpinner.setAdapter(zcAdapter);
        getZCAdapterData();

        commPresenter = new CommFast2Presenter(this,SchedulerProvider.getInstance());
        initTableView();
...
//        Log.i(TAG_NAME,"销毁我了");
     ...
     * @param id1
     */
    @OnItemSelected({R.id.sp_zc_list})
    void OnItemSelected(AdapterView<?> parent, TextView view, int position, long id1) {
        switch (parent.getId()){
            case R.id.sp_zc_list:
                Pair pair=(Pair) parent.getSelectedItem();
                //zcno=pair.getValue();
                System.out.println("ss=zcno="+zcno);
                break;
        }
    ...
    public void getQuickLotBack(Boolean bok, JSONObject batchInfo, String error, int key) {
        if (bok) {
            ...
            record.setSbuid("D0073");//二次过站统一用这个类型
    ...
    public void saveRecordBack(boolean bok, Object records, String error) {
        if (bok) {
            ...
            //commPresenter.changeLotStateOneByOne(record, CommCL.BATCH_STATUS_DONE);//二次清洗预热不需要更新lot表
            hideLoading();
            CommonUtils.canDo(GBKEY);
        } else {
            hideLoading();
